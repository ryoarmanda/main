...
Head over to <<UserGuide.adoc#_command_summary,Command Summary>> for list of commands that are undoable.
...
Example:
Continuing the example in `undo`... now you think that the "bowling" expense is in the right place after all, and you wish to do the addition again.
----
redo
----
Result:
...
Currently, MooLah only supports just a handful of menu items, though. Head over to our <<UserGuide.adoc#_menu_items_list, menu items list>> to see the
